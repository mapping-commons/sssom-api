...
for i in *.ttl.gz; do
    ...
    echo "curl -v --retry 5 --retry-delay 10 -X POST -H \"Content-type: text/turtle\" --data-binary @$i ${RDF4JSERVER}/repositories/sssom/statements?context=null"
    curl -v --retry 5 --retry-delay 10 -X POST -H "Content-type: text/turtle" --data-binary @$i ${RDF4JSERVER}/repositories/sssom/statements?context=null || exit 1
    echo "TIME:"
    date
    sleep 5
done
